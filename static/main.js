...
const SCRIPT_ROOT = JSON.parse(localStorage.getItem("scriptRoot"));
...
async function requestDates(month, year) {
    ...
    let firstYear = new Date(yearKeys[0]).toLocaleString("en-US", {
        year: "numeric",
    });
    let lastYear = new Date(yearKeys[yearKeys.length - 1]).toLocaleString(
        "en-US",
        { year: "numeric" },
    );
...
function createDayElement(theDate) {
    ...
    if (
        standardCommemorations.includes(
            calendarData[dateForJson]["com_1"]["code"],
        )
    ) {
...
function loadDays(increment) {
    ...
    const lastDate =
        document.querySelector(`.calendar:${child}`).firstElementChild
...
const VirtualScroll = ({
    ...
    let visibleNodesCount = Math.ceil(viewportHeight / rowHeight) +
        2 * nodePadding;
...
function updateDOM(direction) {
    ...
    if (direction < 0) {
        refDate = document.querySelector(".calendar").firstElementChild
            .firstElementChild.id;
    } else {
        refDate = document.querySelector(".calendar").lastElementChild
            .firstElementChild.id;
    }
    ...
    CALENDAR.removeChild(weekArray[0]);
...
const dayHeight = window.innerHeight / 6;

function onScroll() {
    if (
        Math.abs(CALENDAR.firstChild.getBoundingClientRect().top) <
            window.innerHeight + dayHeight * 2
    ) {
        updateDOM(-1);
    }
    // if (Math.abs(CALENDAR.firstChild.getBoundingClientRect().top) > 1 * window.innerHeight) {
    if (
        CALENDAR.lastChild.getBoundingClientRect().top <=
...
function displayDetails(date) {
    ...
    detailsPane.querySelector(".details_feast").textContent =
        calendarData[date]["name"];
    detailsPane.querySelector(".details_rank").textContent =
        calendarData[date]["rank"];
    detailsPane.querySelector(".details_color").textContent =
        calendarData[date]["color"];
    detailsPane.querySelector(".details_com_1").textContent =
        calendarData[date]["com_1"]["name"];
    detailsPane.querySelector(".details_com_2").textContent =
        calendarData[date]["com_2"]["name"];
    detailsPane.querySelector(".details_com_3").textContent =
        calendarData[date]["com_3"]["name"];
}
